...
Sub Class_Globals
	...
	Public PressedColor As Int
	Private xui As XUI
	Public DesignerLabel As Label
...
Private Sub SetScrollViewOffset(offset As Float)
	If horizontal Then
		#if B4i
		sv.As(ScrollView).ScrollTo(offset, 0, False)
		#Else
		sv.ScrollViewOffsetX = offset
		#End If
	Else
		#if B4i
		sv.As(ScrollView).ScrollTo(0, offset, False)
		#Else
...
Private Sub PanelClickHandler(SenderPanel As B4XView)
	...
	Dim IsTransparentPressedColor As Boolean = Bit.And(0xff000000, PressedColor) = 0
	If IsTransparentPressedColor = False Then
